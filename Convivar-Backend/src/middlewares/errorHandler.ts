  ...
  if (isHttpError(error)) {
    ...
    if (error.type === "entity.too.large" || statusCode === 413) {
      response.status(413).json({
        message:
          "El archivo contiene demasiados datos para importar en una sola carga. Divide el archivo o reduce la cantidad de filas.",
      });
      return;
    }
  ...
  if (isDatabaseError(error)) {
    if (error.code === "23505") {
      response.status(409).json({
        message:
          "Ya existe un residente con el mismo documento o correo en este conjunto.",
      });
      return;
    }
    ...
    if (error.code === "23503") {
      response.status(400).json({
        message:
          "No fue posible asociar los residentes al conjunto seleccionado.",
      });
      return;
    }
    ...
    if (error.code === "42P01") {
      response.status(500).json({
        message:
          "La base de datos no esta migrada para residentes. Ejecuta las migraciones del backend.",
      });
      return;
    }
  }
